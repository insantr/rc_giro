...
uint8_t rx_tx_cmmd[5] = {0xFC, 0x7E, 0x99, 0x11, 0xC1};
uint8_t rx_tx_bind[5] = {0x65, 0x65, 0x65, 0x65, 0x65};
	...
	uint8_t throttle, rudder=0x00, elevator=0x00, aileron=0x00, rudder_trim, elevator_trim, aileron_trim, flags;
	...
	void setup()
	{
		...
		nrf24.spiWriteRegister( NRF24_REG_04_SETUP_RETR, 0x1F);                           // 500uS timeout, 10 retries
		...
		for(int packno = 0; packno < 60; packno++)
		{
			...
			switch(packwait())
			{
				...
				case PKT_ACK:
				//Serial.println("PKT_ACK");
				break;
				
				case PKT_TIMEOUT:
				//Serial.println("PKT_TIMEOUT");
				break;
			}
		}
	...
	uint8_t i_throttle = 0;
	uint8_t i_aileron = 0;
	uint8_t i_elevator = 0;
	uint8_t i_rudder = 0;
	uint8_t inc = 0;
	...
	void loop()
	{
		...
		if(Serial.available() > 0){
			...
			i_throttle+=1;
			Serial.print("Inc: ");
			Serial.println(i_throttle);
		} else {
			i_throttle=0;
		}

		// Get RX values by PPM, convert to range 0x00 to 0xFF
		throttle = i_throttle;//(uint8_t) (tx.getChannel(0, 1000, 2000, 0x00, 0xFF ));
		aileron  = 0x80;//(uint8_t) (tx.getChannel(1, 1000, 2000, 0x00, 0xFF ));
		elevator = 0x80;//(uint8_t) (tx.getChannel(2, 1000, 2000, 0x00, 0xFF ));
		rudder   = 0x80;//(uint8_t) (tx.getChannel(3, 1000, 2000, 0x00, 0xFF ));
		aux1     = 0x00;//(uint8_t) (tx.getChannel(4, 1000, 2000, 0x00, 0xFF ));
	...
	void send_packet( bool bind )
	{
		...
		Serial.print( "   rudder: " );
		Serial.print( rudder, HEX );
		
		Serial.print( "   elevator: " );
		Serial.print( elevator, HEX );
		
		Serial.print( "   aileron: " );
		Serial.print( aileron, HEX );
		
		Serial.print( "   rudder_trim: " );
		Serial.print( rudder_trim, HEX );
		
		Serial.print( "   elevator_trim: " );
		Serial.print( elevator_trim, HEX );
		
		Serial.print( "   aileron_trim: " );
		Serial.print( aileron_trim, HEX );
